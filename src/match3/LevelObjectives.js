export class LevelObjectives {
  constructor({ targetScore = 500, maxMoves = 20, eggsRequired = 0, boxesRequired = 0, lianaRequired = 0 } = {}) {
    this.targetScore   = targetScore;
    this.maxMoves      = maxMoves;
    this.eggsRequired  = eggsRequired;
    this.boxesRequired = boxesRequired;
    this.lianaRequired = lianaRequired;
    this.score         = 0;
    this.movesUsed     = 0;
    this.eggsBroken    = 0;
    this.boxesBroken   = 0;
  ...
  breakLiana()     { this.lianasBroken = Math.min(this.lianaRequired, this.lianasBroken + 1); }
  ...
  isWon() {
    ...
    const lianaOk  = this.lianaRequired  <= 0 || this.lianasBroken >= this.lianaRequired;
    if (this.eggsRequired > 0 || this.boxesRequired > 0 || this.lianaRequired > 0) return eggsOk && boxesOk && lianaOk;
